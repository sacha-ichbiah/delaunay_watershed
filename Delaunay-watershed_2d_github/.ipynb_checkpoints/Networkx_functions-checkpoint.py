...
def Clean_map_area(List_Graphs,List_Maps, area_min,area_max): 
    graph = List_Graphs[-1]
    Map = List_Maps[-1]
    ...
    G.add_nodes_from(zip(np.arange(len(Filtered_node_list)),areas_dict))
        ...
        for neighbor in Graph.neighbors(node): 
            if node<neighbor : 
                ...
                for key in range(num_edges): 
                    ...
                    length += Graph[node][neighbor][key]['length']
                    weight += Graph[node][neighbor][key]['score']*Graph[node][neighbor][key]['length']

                score = weight/length
                Edges.append((node,neighbor,{'score': score,'length':length}))
...
#SEEDED WATERSHED
#####
def Seeded_Watershed(List_Graphs,List_Maps,seeds,zero_nodes=[]): 
    Graph = List_Graphs[-1]
    Map = seeded_watershed_map(Graph,seeds,zero_nodes)


    reverse_map = Reverse_map(Map)
    Edges=[]
    
    Initial_nodes_areas = Graph.nodes.data('area')
    Cluster_areas=[]

    for i in range(len(Map)): 
        nodes_subset = list(Map[i])
        cluster_area = 0
        for node_idx in nodes_subset : 
             
            cluster_area+=Initial_nodes_areas[node_idx]
    ...
    area_dicts = list(map(lambda x:{'area':x},Cluster_areas))
    Nodes_table = zip(np.arange(len(Map)),area_dicts)
...
def seeded_watershed_map(Graph,seeds,zero_nodes = []): 
    Labels = seeded_watershed_aggregation(Graph,seeds)
    Labels[zero_nodes]=0
    num_label = 0
    for seed in seeds : 
        Labels[seed]=num_label
...
def Sorted_aggregation(List_Graphs,List_Maps,lambda_func): 
    ...
    #mapping = map(zip(sorted(list(Graph.nodes())),np.arange(len(list(Graph.nodes)))))
    
    mapping = dict(zip([u for u,v in Map.items()],np.arange(len(Map))))
...
def contract_nodes(Graph,node_1,node_2):
    ...
    New_Graph.add_node(node_1,area = (Graph.nodes[node_1]['area']+Graph.nodes[node_2]['area']))
    Aggregated_Graph = networkx.MultiGraph()
